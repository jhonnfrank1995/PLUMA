 ...
 * propósito: `Compuertas` no es adyacente a `Redaccion` en la Ley de
...
final class ExtractorJsonLlm {
	 ...
	 * @throws CompuertaException si no se encuentra un objeto JSON válido.
	 */
	public static function extraer( string $contenido ): array {
		...
		if ( false === $inicio || false === $fin || $fin < $inicio ) {
			throw new CompuertaException( 'El proveedor de lenguaje no devolvió un objeto JSON reconocible.' );
		}
		...
		if ( ! is_array( $datos ) ) {
			throw new CompuertaException( 'El proveedor de lenguaje devolvió JSON con formato inesperado.' );
		}
